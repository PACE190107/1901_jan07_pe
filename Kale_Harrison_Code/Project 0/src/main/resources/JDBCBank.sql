...
---Procedure for super view
DROP PROCEDURE SUPERVIEW;
CREATE OR REPLACE PROCEDURE superView
AS
BEGIN
select USERNAME, PASSPHRASE, BANK_USER.USER_ID, SUPER, 
ACCOUNT_TYPE, ACCOUNT_ID, BALANCE
FROM BANK_USER FULL JOIN BANK_ACCOUNT
on BANK_USER.USER_ID = BANK_ACCOUNT.USER_ID
order by BANK_USER.USERNAME;
COMMIT;
END;
/
