...
--    PersonID int FOREIGN KEY REFERENCES Persons(PersonID)
    ACCOUNT_ID NUMBER REFERENCES BANKING_ACCOUNT(ACCOUNT_ID) ON DELETE CASCADE,
    CUSTOMER_ID NUMBER REFERENCES BANKING_CUSTOMER(CUSTOMER_ID) ON DELETE CASCADE,
...
ALTER TABLE TRANSACTION_LOG ADD BALANCE_CHANGE NUMBER;
...
CREATE OR REPLACE PROCEDURE GET_HISTORY(
    ACCOUNTID NUMBER, RETURNTABLE OUT SYS_REFCURSOR)
    IS BEGIN
    OPEN RETURNTABLE FOR SELECT
...
--LOGGING
CREATE OR REPLACE TRIGGER TRANSACTION_LOGGER
BEFORE UPDATE ON BANKING_ACCOUNT
FOR EACH ROW
BEGIN
    INSERT INTO TRANSACTION_LOG VALUES( :NEW.ACCOUNT_ID, :OLD.BALANCE,SYSDATE, :NEW.BALANCE-:OLD.BALANCE );
END;
/
